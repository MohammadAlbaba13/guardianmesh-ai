...
import './domains.css';
import './mission.css';
import {MissionControl, NetworkEvidence, IncidentReplay, defaultOptions} from './components/MissionControl';
...
export default function App() {
  ...
  const effectiveAutoApprove=autoApprove&&options.execution_mode==='SIMULATION';
  const runOptions={...options,manual_approval:!effectiveAutoApprove};
  const launch=()=>incident?engine.restart(scenario,mode,effectiveAutoApprove,runOptions):engine.start(scenario,mode,effectiveAutoApprove,runOptions);
  const hero=async()=>{await engine.switchDomain('smart_city');setScenario('camera');await engine.restart('camera',mode,effectiveAutoApprove,runOptions);};
  ...
  return <div className={`app-shell ${judge?'judge-mode':''}`}><header className="app-header"><a className="brand" href="/" aria-label="GuardianMesh AI command center"><span className="brand-mark"><ShieldCheck size={25}/></span><span>GuardianMesh<span className="brand-ai">AI</span><small>RESILIENCE & TRUST PLATFORM</small></span></a><div className="header-nav"><span className="nav-active">Command center</span><button onClick={()=>setShowReports(true)}><FileText size={15}/>Incident reports</button></div><div className="system-indicators"><span><i className={`status-dot ${engine.ready?'':'amber'}`}/>{engine.ready?'Resilience engine ready':'Connecting engine'}</span><span className="header-local"><Globe2 size={14}/>LOCAL PROTOTYPE</span></div></header>
    <main><div className="platform-intro"><div><span className="eyebrow">GUARDIANMESH AI</span><h1>Autonomous Multi-Domain<br className="hero-break"/> Resilience & Trust Platform</h1><p>One intelligence engine. Seven MENA domains. One programmable network.</p></div><div className="event-brand"><span>MENA IGNITE 2026</span><p>Policy-governed intelligence</p><small>Local, deterministic, auditable</small></div></div>
      ...
      <section className="demo-controls" aria-label="Demo controls"><div className="scenario-control"><span className="control-icon"><Radio size={18}/></span><label htmlFor="scenario">{domain?.event_label??'EVENT'} SCENARIO<select id="scenario" value={scenario} onChange={e=>setScenario(e.target.value)} disabled={active||engine.busy}>{engine.scenarios.length?engine.scenarios.map((s,i)=><option value={s.id} key={s.id}>{String(i+1).padStart(2,'0')} · {s.title}</option>):<option value="">Loading scenarios</option>}</select></label></div><div className="mode-control" role="group" aria-label="Demo mode"><button className={mode==='fast'?'selected':''} disabled={active} onClick={()=>setMode('fast')} aria-pressed={mode==='fast'}><Zap size={14}/>Fast pitch<span>20–35 sec</span></button><button className={mode==='guided'?'selected':''} disabled={active} onClick={()=>setMode('guided')} aria-pressed={mode==='guided'}><Play size={13}/>Guided<span>~1 min</span></button></div><label className="approval-toggle" title="Simulation only. LIVE and AUTO always require an operator decision. Simulated automatic decisions are recorded as DEMO_AUTOMATION."><input type="checkbox" checked={effectiveAutoApprove} onChange={e=>setAutoApprove(e.target.checked)} disabled={active||options.execution_mode!=='SIMULATION'}/><span className="toggle-switch"><Check size={11}/></span><span>Demo auto-approval</span></label><button className="button primary run-button" disabled={!canStart} onClick={()=>void launch()}>{engine.busy?<LoaderCircle size={17} className="spin"/>:<Play size={16} fill="currentColor"/>}{active?'Autonomous defense running':'Run Autonomous Defense Demo'}</button></section>
      <MissionControl options={options} onChange={setOptions} active={active||engine.busy} judge={judge} onJudge={()=>setJudge(!judge)} connection={engine.connection} onHero={hero} onReset={()=>engine.control('reset')} canRun={engine.ready&&!engine.busy&&!active}/>
      {judge&&active&&<div className="judge-action-rail"><span>{incident?.status==='AWAITING_APPROVAL'?'POLICY VALIDATED · OPERATOR DECISION REQUIRED':`INCIDENT ${incident?.phase??''}`}</span>{incident?.status==='AWAITING_APPROVAL'?<><button className="button primary small" disabled={engine.busy||!!incident.approvals.length} onClick={()=>void engine.approve('APPROVE')}>Approve response</button><button className="button small" disabled={engine.busy||!!incident.approvals.length} onClick={()=>void engine.approve('REJECT')}>Reject response</button></>:<><button className="button small" disabled={engine.busy||!['RUNNING','PAUSED'].includes(incident?.status??'')} onClick={()=>void engine.control(incident?.status==='PAUSED'?'resume':'pause')}>{incident?.status==='PAUSED'?'Resume incident':'Pause incident'}</button><button className="button small" disabled={engine.busy||incident?.status!=='RUNNING'} onClick={()=>void engine.control('skip')}>Skip timed step</button></>}</div>}
      <Continuity topology={engine.topology} incident={incident} domain={domain}/><Storyline incident={incident}/>
      <div className="workspace-grid"><div className="network-column">{engine.topology?<Topology key={`${domain?.id}-${incident?.id??'standby'}`} topology={engine.topology} incident={incident} domain={domain}/>:<section className="panel twin-loading"><RadarPlaceholder/><h2>Connecting to the digital twin</h2><p>Waiting for the local defense engine.</p></section>}<div className="playback-bar"><div className="stream-state"><i className={`status-dot ${['live','ready'].includes(engine.connection)?'':'amber'}`}/>{statusLabel}{incident&&<span className="phase-label">{incident.status==='PAUSED'?'PAUSED':incident.phase.replace('_',' ')}</span>}</div><div className="playback-buttons"><button disabled={!incident||engine.busy||!['RUNNING','PAUSED'].includes(incident.status)} onClick={()=>void engine.control(incident?.status==='PAUSED'?'resume':'pause')}>{incident?.status==='PAUSED'?<Play size={14}/>:<Pause size={14}/>} {incident?.status==='PAUSED'?'Resume':'Pause'}</button><button disabled={!incident||engine.busy||incident.status!=='RUNNING'} onClick={()=>void engine.control('skip')}><SkipForward size={14}/>Skip</button><button disabled={!incident||engine.busy} onClick={()=>void engine.restart(scenario,mode,effectiveAutoApprove,runOptions)}><RotateCcw size={14}/>Restart</button><button disabled={!incident||engine.busy} onClick={()=>void engine.control('reset')}><RefreshCw size={14}/>Reset</button></div></div></div><ThreatPanel domain={domain} incident={incident} topology={engine.topology} approve={engine.approve} busy={engine.busy} onReport={()=>setShowReports(true)}/></div>
      <NetworkEvidence incident={incident}/><IncidentReplay incident={incident}/>
      ...
      <footer className="app-footer"><span><ShieldCheck size={14}/>GuardianMesh AI <i/>Policy-governed resilience</span><span>Offline-ready · Evidence-driven outcomes · Optional local AI</span><a href="/api/docs" target="_blank" rel="noreferrer">API reference<ArrowUpRight size={13}/></a></footer>
